...
- **`package.json`** (repo root): `packageManager` pin, the `check:dep-age`/`lint`/`typecheck`/`test`/`format`/`format:check`/`prepare` scripts, and `husky` + `prettier` + `@crm/config` as devDependencies; not a workspace package itself
