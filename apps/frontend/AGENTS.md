...
After changes, run from the repo root (or `apps/frontend/` without `--filter crm-frontend`):

```bash
pnpm install
pnpm --filter crm-frontend run lint
pnpm --filter crm-frontend exec tsc --noEmit
pnpm --filter crm-frontend exec tsc --noEmit -p tsconfig.node.json
pnpm --filter crm-frontend run test
